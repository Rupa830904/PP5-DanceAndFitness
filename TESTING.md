...
## BUGS SOLVED BEFORE RE-SUBMIT

* **Internal server error 500**
    When a unauthorised user tries to checkput, Internal server error present in application. 
    Solution:
    Bug in the code found in 'checkout_success' view. Fixed by adding 'if' clause for authorised user.
* **Checkout not completed**
    User did not recieve confirmation email after checkout 
    Solution:
    Webhook delivery was failing due errors in 'payment_intent_success' webhook handler.Errors fixed after debugging
